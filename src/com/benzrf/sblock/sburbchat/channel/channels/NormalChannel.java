...
public class NormalChannel implements Channel, Serializable
{
	...
	public void setChat(String m, User sender) 
	{
		...
		this.sendToAll(this.getChatPrefix(sender, m) + ((m.startsWith("\\#") || m.startsWith("#")) ? m.substring(1) : m), sender);
	}
	...
	public String getChatPrefix(User sender, String message)
	{
		...
		if (sender.hasPermission("sburbchat.darkred"))
			colorP = ChatColor.DARK_RED;
		else if (sender.hasPermission("sburbchat.blue"))
			colorP = ChatColor.BLUE;
		else if (sender.hasPermission("sburbchat.gold"))
			colorP = ChatColor.GOLD;
		else if (sender.hasPermission("sburbchat.green"))
			colorP = ChatColor.GREEN;
		
		if (sender.getPlayerWorld(sender.getPlayer()).equalsIgnoreCase("earth"))
			colorW = ChatColor.DARK_GREEN;
		else if (sender.getPlayerWorld(sender.getPlayer()).equalsIgnoreCase("innercircle"))
			colorW = ChatColor.YELLOW;
		else if (sender.getPlayerWorld(sender.getPlayer()).equalsIgnoreCase("outercircle"))
			colorW = ChatColor.DARK_PURPLE;
		else if (sender.getPlayerWorld(sender.getPlayer()).equalsIgnoreCase("furthestring"))
			colorW = ChatColor.DARK_GRAY;
		
		return (message.startsWith("#") ? "* " : colorW + "<") + colorP + sender.getDisplayName() + ChatColor.WHITE + (message.startsWith("#") ? "" : colorW + "> " + ChatColor.WHITE);
	}
